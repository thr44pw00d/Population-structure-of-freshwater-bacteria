...
>>>> The script was slightly modified by Matthias Hötzinger to add numbers of mapped bases.
"""
import sys
import os
import argparse
...
def get_bedcov_dict(bedcoverage):
    ...
    for line in fh:
        ...
        if int(cols[1]) == 0:
            ...
            d["mapped_bases"] = d.get("mapped_bases", 0) + int(cols[1]) * int(cols[2])
...
def print_input_table(fastadict, bedcovdicts, samplenames=None):
    ...
    if samplenames is None:
        ...
        print_sample_columns(tuple(range(len(bedcovdicts))), "mapped_bases_sample_")
    else:
        ...
        print_sample_columns(tuple(samplenames), "mapped_bases_sample_")
    sys.stdout.write("\n")
    ...
    for acc in fastadict:
        ...
        for bcd in bedcovdicts:
            try:
                # Print mapped bases
                sys.stdout.write("\t%f" % (bcd[acc]["mapped_bases"]))
            except KeyError:
                # No reads mapped to this contig
                sys.stdout.write("\t0")
