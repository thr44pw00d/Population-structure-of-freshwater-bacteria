...
FE_r <- read.csv("/media/matthias/meuk/intercontinental/pogenom_cov30_mc15/008_allele_freqs/Fsp_Erken_r_unique.tsv", header=TRUE, sep="\t", check.names=F)
...
### 11 clusters according to PBC (Point biserial correlation)

#cut into "strains"
strains <- cutree(hc, k=11)

#color "strains" in tree
library(dendextend)
avg_dend_obj <- as.dendrogram(hc)
avg_col_dend <- color_branches(avg_dend_obj, k=11)
pdf(file="/media/matthias/meuk/intercontinental/pogenom_cov30_mc15/008_allele_freqs/Fsp_Erk_hc_11strains.pdf", width=35, height=15)
plot(avg_col_dend)
dev.off()
...
h <- hist(strains_df$strains, breaks=0:11)
...
#subset strains (from the one with most alleles as strain1 to the one with least alleles to strain11)
strain1 <- subset(strains_df, strains==7)
strain2 <- subset(strains_df, strains==6)
strain3 <- subset(strains_df, strains==3)
strain4 <- subset(strains_df, strains==5)
strain5 <- subset(strains_df, strains==4)
strain6 <- subset(strains_df, strains==1)
strain7 <- subset(strains_df, strains==11)
strain8 <- subset(strains_df, strains==8)
strain9 <- subset(strains_df, strains==9)
strain10 <- subset(strains_df, strains==2)
strain11 <- subset(strains_df, strains==10)

# laod allele freqs
all <- read.csv("/media/matthias/meuk/intercontinental/pogenom_cov30_mc15/008_allele_freqs/Fsp_Erken_all_unique.csv", header=TRUE, sep="\t", check.names=F)
...
strain10_freqs <- all2[rownames(all2) %in% strain10$alleles, ]
strain11_freqs <- all2[rownames(all2) %in% strain11$alleles, ]
...
# make freqs numeric
s1_avg$freq <- as.numeric(as.character(s1_avg$freq))
# date as date
...
# make freqs numeric
s2_avg$freq <- as.numeric(as.character(s2_avg$freq))
# date as date
...
# make freqs numeric
s3_avg$freq <- as.numeric(as.character(s3_avg$freq))
# date as date
...
# make freqs numeric
s4_avg$freq <- as.numeric(as.character(s4_avg$freq))
# date as date
...
# make freqs numeric
s5_avg$freq <- as.numeric(as.character(s5_avg$freq))
# date as date
...
# make freqs numeric
s6_avg$freq <- as.numeric(as.character(s6_avg$freq))
# date as date
...
# make freqs numeric
s7_avg$freq <- as.numeric(as.character(s7_avg$freq))
# date as date
...
# make freqs numeric
s8_avg$freq <- as.numeric(as.character(s8_avg$freq))
# date as date
...
# make freqs numeric
s9_avg$freq <- as.numeric(as.character(s9_avg$freq))
# date as date
...
### Strain 10
# calculate mean (median) of each row
s10_avg <- as.data.frame(apply(strain10_freqs, 2, mean))
#s10_med <- as.data.frame(apply(strain10_freqs, 2, median))
# rownames to col1
s10_avg <- tibble::rownames_to_column(s10_avg)
# set colnames
colnames(s10_avg) <- c("date", "freq")
# make freqs numeric
s10_avg$freq <- as.numeric(as.character(s10_avg$freq))
# date as date
s10_avg$date <- as.Date(s10_avg$date, format='%Y-%m-%d')
s10_avg$date <- format(s10_avg$date, "%d-%b-%Y")
# make date an ordered factor
s10_avg$date <- factor(s10_avg$date, levels = s10_avg$date)

### Strain 11
# calculate mean (median) of each row
s11_avg <- as.data.frame(apply(strain11_freqs, 2, mean))
#s11_med <- as.data.frame(apply(strain11_freqs, 2, median))
# rownames to col1
s11_avg <- tibble::rownames_to_column(s11_avg)
# set colnames
colnames(s11_avg) <- c("date", "freq")
# make freqs numeric
s11_avg$freq <- as.numeric(as.character(s11_avg$freq))
# date as date
s11_avg$date <- as.Date(s11_avg$date, format='%Y-%m-%d')
s11_avg$date <- format(s11_avg$date, "%d-%b-%Y")
# make date an ordered factor
s11_avg$date <- factor(s11_avg$date, levels = s11_avg$date)

abu_sum = sum(abus_sort$Freq)
abu11 = nrow(strain11)/abu_sum
abu10 = nrow(strain10)/abu_sum
...
library(ggplot2)
# open pdf
pdf(file="/media/matthias/meuk/intercontinental/Fsp_Erk_dynamics.pdf", width=6, height=4.5)
# plot
ggplot() +
      theme_bw() +
...
#strain11
      geom_point(data=s11_avg, size=abu11*30, color=col11, alpha=abu11/abu1, aes(x=date, y=freq)) +
      geom_line(data=s11_avg, color=col11, size=abu11*7, alpha=abu11/abu1, aes(x=date, y=freq, group=1)) +
#strain10
      geom_point(data=s10_avg, size=abu10*30, color=col10, alpha=abu10/abu1, aes(x=date, y=freq)) +
      geom_line(data=s10_avg, color=col10, size=abu10*7, alpha=abu10/abu1, aes(x=date, y=freq, group=1)) +
#strain9
